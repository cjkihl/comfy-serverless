...
1. **Proxy Server**: Running locally on `PROXY_URL` (default: `ws://localhost:8190/ws`)
2. **ComfyUI Server**: Running on **remote server** configured via proxy's `PROXY_COMFY_URL` environment variable
3. **Environment Variables**: Set `PROXY_COMFY_JWT_SECRET` in your `.env` file or environment

### Environment Variables

The tests use `with-env` to automatically load environment variables from `.env` files. Create a `.env` file in the project root:
...
**Note**: The `PROXY_COMFY_JWT_SECRET` must match the proxy's `PROXY_COMFY_JWT_SECRET` for authentication to work.
...
- `generateJWT(userId, secret, expiresInSeconds)` - Creates signed HS256 JWT tokens
