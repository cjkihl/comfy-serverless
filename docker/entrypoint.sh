...
REPO_URL="https://github.com/cjkihl/comfy-serverless.git"
CODE_DIR="/comfy"
REPO_REF=${REPO_REF:-}
...
if [[ -f "$CODE_DIR/scripts/install_requirements.sh" ]]; then
    ...
    echo "[entrypoint] Installing requirements from requirements.txt via Pixi ..."
    pixi run python -m pip install -r "$CODE_DIR/requirements.txt"
...
echo "Delegating boot to repo script: scripts/container_boot.ts"
exec pixi run bash -lc "cd '$CODE_DIR' && bun run ./scripts/container_boot.ts"
